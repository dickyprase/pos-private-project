    ...
    <header class="z-30 shrink-0 border-b border-stone-200 bg-white/95 backdrop-blur">
        <div class="flex min-h-16 items-center gap-3 px-3 py-2 sm:px-5">
            ...
            <div class="ml-auto flex items-center gap-2">
                ...
                @endif
                <div data-pos-datetime class="relative rounded-xl bg-gradient-to-br from-brand-500 via-brand-200 to-white p-[1px] shadow-[0_8px_24px_rgba(249,115,22,0.15)]">
            ...
            <section data-payment-dialog x-show="shown" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 translate-y-3" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95 translate-y-2" class="relative z-10 flex max-h-[92dvh] w-full max-w-xl flex-col overflow-hidden rounded-t-[28px] bg-[#fafafa] shadow-2xl sm:rounded-[28px]" role="dialog" aria-modal="true" aria-labelledby="paymentTitle">
                ...
                <div class="scrollbar-thin min-h-0 space-y-5 overflow-y-auto p-5">
                    ...
                    @if($paymentMethod === 'CASH')
                        <div class="space-y-3" x-data="currencyInput({{ $receivedAmount }}, {{ $grandTotal }})">
                            <label class="block"><span class="mb-2 block text-sm font-bold">Uang diterima</span><div class="relative"><span class="absolute left-4 top-1/2 -translate-y-1/2 text-sm font-semibold text-stone-500">Rp</span><input x-ref="receivedInput" @input="handleReceivedInput($event)" x-init="value = displayReceived" type="text" inputmode="numeric" autocomplete="off" value="{{ number_format($receivedAmount, 0, ',', '.') }}" class="h-14 w-full rounded-2xl border border-stone-200 bg-stone-50 pl-11 pr-4 text-right text-xl font-bold tabular-nums focus:bg-white"></div></label>
                            <div class="grid grid-cols-3 gap-2">@foreach($cashOptions as $cash)<button type="button" @click="setReceived({{ $cash }})" class="pressable min-h-11 rounded-xl border border-stone-200 bg-white px-2 text-xs font-bold hover:bg-stone-50">{{ $cash === $grandTotal ? 'Uang pas' : number_format($cash, 0, ',', '.') }}</button>@endforeach</div>
                            <div class="flex items-center justify-between rounded-2xl bg-emerald-50 px-4 py-3"><span class="text-sm font-semibold text-emerald-800">Kembalian</span><span class="font-black text-emerald-800">Rp <span x-text="displayChange">{{ number_format(max(0, $receivedAmount - $grandTotal), 0, ',', '.') }}</span></span></div>
                        </div>
